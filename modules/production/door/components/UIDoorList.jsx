...
export default function UIDoorList({
  headerContent,
  doors: rawDoors = [],
  isLoading,
}) {
  const doors = rawDoors.map((r) => ({
    ...
    creator:
      [r.createdBy?.userFirstName, r.createdBy?.userLastName]
        .filter(Boolean)
        .join(" ") || "-",
    createAt: dateToThai(r.doorCreateAt),
    updateBy:
      [r.updatedBy?.userFirstName, r.updatedBy?.userLastName]
        .filter(Boolean)
        .join(" ") || "-",
    updateAt: dateToThai(r.doorUpdateAt),
    status: r.doorStatus?.toLowerCase() || "enable",
  }));
  return (
    <>
      ...
      <div className="flex items-center justify-center w-full p-2 gap-2">
        <UITable
          ...
          entityNamePlural="doors"
          searchPlaceholder="Search by door name..."
          basePath="/door"
        />
      </div>
    </>
